...
st.bar_chart(data1["diagnosis"])


st.write("Histogram")

df = pd.DataFrame(data1[:200], columns = ["radius_mean","radius_se","radius_worst","texture_mean"])
df.hist()
plt.show()
st.pyplot(plt)
...
st.write("Area Chart")
chart_data = pd.DataFrame(data1[:40], columns=["radius_mean", "texture_mean"])
...
with dataset:
    ...
    dataset_name = st.sidebar.selectbox("Select Dataset",("Breast Cancer","Datset","Dataset"))
